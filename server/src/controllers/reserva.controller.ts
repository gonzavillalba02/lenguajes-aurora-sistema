...
async function getEstadoId(nombre: string): Promise<number> {
  const [rows]: any = await pool.query("SELECT id FROM estado_reserva WHERE nombre = ? LIMIT 1", [nombre]);
...
async function haySolapamiento(habitacionId: number, fechaInicio: string, fechaFin: string): Promise<boolean> {
  // Consideramos ocupadas las reservas aprobadas y también las pendientes/pago (bloquean calendario del operador)
  const estadosBloqueantes = ["pendiente_verificacion", "pendiente_pago", "aprobada"];
...
export const crearReservaPublica = async (req: Request, res: Response) => {
  try {
    const {
      ...
      nombre, apellido, email, ubicacion, telefono,
      // reserva
      habitacion_id, fecha_inicio, fecha_fin, observaciones,
    } = req.body;

    if (!nombre || !apellido || !email || !habitacion_id || !fecha_inicio || !fecha_fin) {
      return res.status(400).json({ message: "Faltan datos obligatorios (nombre, apellido, email, habitacion_id, fecha_inicio, fecha_fin)" });
    }
    if (!fechasValidas(fecha_inicio, fecha_fin)) {
      return res.status(400).json({ message: "Rango de fechas inválido (fecha_inicio > fecha_fin)" });
    }

    // persona: crear/usar por email
    const personaId = await ensurePersona({ nombre, apellido, email, ubicacion, telefono });
    ...
    const [ins]: any = await pool.query(
      `INSERT INTO reserva 
       ...
       VALUES (?, ?, ?, ?, ?, ?, NULL, NULL)`,
      [personaId, habitacion_id, fecha_inicio, fecha_fin, estadoId, observaciones || null]
    );

    res.status(201).json({ message: "Reserva creada (pendiente de verificación)", reservaId: ins.insertId, personaId });
  } catch (err: any) {
    console.error(err);
...
export const crearReservaOperador = async (req: Request, res: Response) => {
  try {
    ...
    if (!habitacion_id || !fecha_inicio || !fecha_fin) {
      return res.status(400).json({ message: "Faltan datos obligatorios (habitacion_id, fecha_inicio, fecha_fin)" });
    }
    if (!fechasValidas(fecha_inicio, fecha_fin)) {
      return res.status(400).json({ message: "Rango de fechas inválido (fecha_inicio > fecha_fin)" });
    }
    ...
    if (persona_id) {
      ...
      const [chk]: any = await pool.query("SELECT id FROM persona WHERE id = ? LIMIT 1", [personaId]);
      if (!chk.length) return res.status(400).json({ message: "persona_id no existe" });
    } else {
      // si no mandan persona_id, exigimos datos mínimos para crear/usar por email
      if (!nombre || !apellido || !email) {
        return res.status(400).json({ message: "Faltan datos de persona (nombre, apellido, email) o persona_id" });
      }
      personaId = await ensurePersona({ nombre, apellido, email, ubicacion, telefono });
    }
    ...
    const solapa = await haySolapamiento(Number(habitacion_id), fecha_inicio, fecha_fin);
    if (solapa) {
      return res.status(409).json({ message: "Solapamiento de fechas para la habitación seleccionada" });
    }
    ...
    const [ins]: any = await pool.query(
      `INSERT INTO reserva
        (persona_id, habitacion_id, fecha_inicio, fecha_fin, estado_id, observaciones, creada_por, aprobada_por)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [personaId, habitacion_id, fecha_inicio, fecha_fin, estadoId, observaciones || null, usuarioId, usuarioId]
    );

    res.status(201).json({ message: "Reserva creada y aprobada", reservaId: ins.insertId, personaId });
  } catch (err: any) {
    console.error(err);
...
export const pasarAPendientePago = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const pendienteVerif = estados.find((e: any) => e.nombre === "pendiente_verificacion");
    const pendientePago = estados.find((e: any) => e.nombre === "pendiente_pago");

    if (!pendienteVerif || !pendientePago) {
      return res.status(500).json({ message: "No se encontraron los estados requeridos" });
    }
    ...
    if (reserva[0].estado_id !== pendienteVerif.id) {
      return res.status(400).json({
        message: "Solo se puede pasar a 'pendiente_pago' si la reserva está en 'pendiente_verificacion'"
      });
    }

    // Actualizar estado
    await pool.query(
      "UPDATE reserva SET estado_id = ? WHERE id = ?",
      [pendientePago.id, id]
    );
...
export const aprobarReserva = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!EST.aprobada || !EST.rechazada || !EST.cancelada) {
      return res.status(500).json({ message: "Estados requeridos no configurados" });
    }
    ...
    if (r.estado_id === EST.rechazada || r.estado_id === EST.cancelada)
      return res.status(400).json({ message: "No se puede aprobar una reserva rechazada o cancelada" });
    ...
    if (overlap.length)
      return res.status(409).json({ message: "Hay una reserva aprobada que solapa en fechas para esta habitación" });
...
export const rechazarReserva = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!EST.rechazada) return res.status(500).json({ message: "Estado 'rechazada' no configurado" });

    // Buscar reserva
    const [rows]: any = await pool.query("SELECT estado_id FROM reserva WHERE id = ?", [id]);
    if (!rows.length) return res.status(404).json({ message: "Reserva no encontrada" });
    ...
    if (estadoActual === EST.aprobada) {
      return res.status(400).json({ message: "No se puede rechazar una reserva ya aprobada" });
    }
    if (estadoActual === EST.cancelada) {
      return res.status(400).json({ message: "No se puede rechazar una reserva cancelada" });
    }

    // Actualizar a rechazada
    await pool.query("UPDATE reserva SET estado_id = ? WHERE id = ?", [EST.rechazada, id]);
...
export const cancelarReserva = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!EST.cancelada) return res.status(500).json({ message: "Estado 'cancelada' no configurado" });
    ...
    if (estadoActual === EST.rechazada) {
      return res.status(400).json({ message: "No se puede cancelar una reserva rechazada" });
    }

    // Actualizar a cancelada + registrar usuario
    await pool.query("UPDATE reserva SET estado_id = ?, aprobada_por = ? WHERE id = ?", [
      EST.cancelada,
      usuarioId,
      id,
    ]);
