...
export const crearHabitacion = async (req: Request, res: Response) => {
  const { nombre, tipo_id, disponible = true, activa = true, observaciones = null } = req.body;

  if (!nombre || !tipo_id) {
    return res.status(400).json({ message: "Faltan datos obligatorios (nombre, tipo_id)" });
  }
  ...
  try {
    ...
    if (error.code === "ER_DUP_ENTRY") {
      return res.status(400).json({ message: "El nombre de la habitación ya existe" });
    }
...
export const desactivarHabitacion = async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const [rows]: any = await pool.query("SELECT activa FROM habitacion WHERE id = ?", [id]);
    if (rows.length === 0) return res.status(404).json({ message: "Habitación no encontrada" });

    if (!rows[0].activa) return res.status(400).json({ message: "La habitación ya está desactivada" });

    await pool.query("UPDATE habitacion SET activa = FALSE WHERE id = ?", [id]);
...
export const reactivarHabitacion = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (rows[0].activa) return res.status(400).json({ message: "La habitación ya está activa" });

    await pool.query("UPDATE habitacion SET activa = TRUE WHERE id = ?", [id]);
...
export const bloquearHabitacion = async (req: Request, res: Response) => {
  const { id } = req.params;
  try {
    const [rows]: any = await pool.query("SELECT disponible FROM habitacion WHERE id = ?", [id]);
    if (rows.length === 0) return res.status(404).json({ message: "Habitación no encontrada" });

    if (!rows[0].disponible) return res.status(400).json({ message: "La habitación ya está bloqueada" });

    await pool.query("UPDATE habitacion SET disponible = FALSE WHERE id = ?", [id]);
...
export const desbloquearHabitacion = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (rows[0].disponible) return res.status(400).json({ message: "La habitación ya está disponible" });

    await pool.query("UPDATE habitacion SET disponible = TRUE WHERE id = ?", [id]);
